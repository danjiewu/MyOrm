...
namespace Northwind.RemoteAPI
{
    ...
    public class HttpDispatcher : IRemoteDispatcher
    {
        ...
        {
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(Settings.Default.Sample4_Northwind_RemoteHttpHandleUrl);
            request.Method = "POST";
    ...
    public class WebServiceDispatcher : IRemoteDispatcher
    {
        ...
        public object RemoteInvoke(string serviceName, MethodInfo method, object[] args)
        {
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                logger.DebugFormat("DAO:{{{0}}} Method:{{{1}}} finished. Returns:{{{2}}}. {3} bytes recieved.", serviceName, method.Name, ret, output.Length);
                return ret;
            }
        }
