...
namespace MyOrm
{
    public class SqlBuilder
    {
        ...
        {
            return String.Format("select * from (select {0}, Row_Number() over (Order by {1}) as Row_Number from {2} where {3}) as TempTable where Row_Number > {4} and Row_Number <= {5}", select, from, where, orderBy, startIndex, startIndex + sectionSize);
        }
